...
function renderHomeAssistantCard(day) {
    const metrics = day.home_assistant_metrics || [];
    if (!metrics.length) return '';
    const formatMetric = (value, unit) => {
        if (value === null || value === undefined) return '-';
        const rounded = Number(value).toFixed(1).replace(/\.0$/, '');
        return `${rounded}${escapeHtml(unit || '')}`;
    };
    const rows = metrics.map(metric => `
        <div class="metric-row">
            <span class="metric-label">${escapeHtml(metric.display_name)}</span>
            <span class="metric-value">${formatMetric(metric.value, metric.unit)}</span>
        </div>
        <div class="metric-row">
            <span class="metric-label">Min / max</span>
            <span class="metric-value">${formatMetric(metric.min, metric.unit)} / ${formatMetric(metric.max, metric.unit)}</span>
        </div>
        <div class="metric-row">
            <span class="metric-label">Coverage</span>
            <span class="metric-value">${Math.round(metric.coverage_pct)}% coverage</span>
        </div>
    `).join('');
    ...
    return `
        <div class="metric-card">
            ...
            <div class="primary-metric">
                <span class="metric-value">${formatMetric(first.value, first.unit)}</span>
                <span class="metric-unit">${escapeHtml(first.display_name)}</span>
            </div>
            ${renderMetricDetails(rows, 'home-assistant')}
        </div>
    `;
}
